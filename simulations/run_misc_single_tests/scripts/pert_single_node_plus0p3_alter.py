...
def pert(t, system):
    ...
    if (not _s["applied"]) and t>=5.0:
        p_new=_s["p0"]+_s["dP"]
        q_new=_s["q0"]+_s["dQ"]
        # use alter() to apply runtime parameter changes
        system.PQ.alter(src='p0', idx=_s['idx'], value=p_new)
        system.PQ.alter(src='q0', idx=_s['idx'], value=q_new)
        system.TDS.custom_event=True
        _s["applied"]=True
        print(f"[pert:event] t={t:.4f}s {_s['idx']} load +0.3 via alter => p0={p_new:.5f}, q0={q_new:.5f}")
